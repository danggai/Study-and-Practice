...
class VPAdapter(private val items: List<Data>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = when(viewType) {
        TYPE_SONG -> SongViewHolder(LayoutInflater.from(parent.context)
            .inflate(R.layout.activity_vp_item, parent, false))
        TYPE_SECTION -> SectionViewHolder(LayoutInflater.from(parent.context)
            .inflate(R.layout.activity_vp_section, parent, false))
        else -> throw IllegalArgumentException()
    }
